...
const LINK_CACHE_SCHEMA_VERSION = 5;
...
export async function writeLinkCache(repoPath: string, commits: CommitWithLinks[]): Promise<void> {
  ...
  const entry: LinkCacheEntry = {
    schemaVersion: LINK_CACHE_SCHEMA_VERSION,
...
export async function readLinkCache(repoPath: string): Promise<LinkCacheEntry | null> {
  try {
    ...
    const parsed = JSON.parse(raw) as LinkCacheEntry;
    return parsed.schemaVersion === LINK_CACHE_SCHEMA_VERSION ? parsed : null;
  } catch {
    return null;
  }
}
...
export async function readAllLinkCaches(): Promise<LinkCacheEntry[]> {
  ...
  for (const file of files) {
    ...
    try {
      ...
      const parsed = JSON.parse(raw) as LinkCacheEntry;
      if (parsed.schemaVersion === LINK_CACHE_SCHEMA_VERSION) results.push(parsed);
    } catch {
      // corrupt file — skip
    }
  }
