...
async function parseSession(
  ...
  try {
    ...
    for (const line of content.split('\n')) {
      ...
      if (type === 'event_msg' && payload?.['type'] === 'patch_apply_end' && pendingUserMessage !== null) {
        const callId = payload['call_id'] as string | undefined;
        const changes = payload['changes'];
        if (changes && typeof changes === 'object') {
          for (const f of Object.keys(changes as Record<string, unknown>)) {
            pendingTouchedFiles.add(path.isAbsolute(f) ? f : (cwd ? path.resolve(cwd, f) : f));
          }
        }
        if (callId) {
          const idx = pendingCallIndex.get(callId);
          const target = idx === undefined ? undefined : pendingItems[idx];
          if (target?.kind === 'tool') {
            const stdout = typeof payload['stdout'] === 'string' ? payload['stdout'] : '';
            target.tool.output = truncate(stdout, maxChars);
          }
        }
      ...
      if (type === 'session_meta' && payload) {
        ...
        const head = git?.['commit_hash'];
        if (typeof head === 'string') {
          for (const sha of extractShas(head)) referencedCommits.add(sha);
        }
      ...
      // Tool call (custom_tool_call). Codex records apply_patch here in recent
      // builds, so this is the primary edit signal for many sessions.
      if (itemType === 'custom_tool_call' && pendingUserMessage !== null) {
        const name = (payload['name'] as string | undefined) ?? '?';
      ...
      // Tool result (function_call_output / custom_tool_call_output)
      if (
        ...
        for (const sha of extractShas(outStr)) referencedCommits.add(sha);
...
function extractEditedFilesFromCodexCall(
  ...
  if (name === 'apply_patch') {
    const patch = typeof input['input'] === 'string' ? input['input'] as string : '';
    return extractEditedFilesFromPatchText(patch, cwd);
  }
  ...
  if (name === 'shell_command' || name === 'shell') {
    ...
    return extractEditedFilesFromPatchText(cmd, cwd);
  }
...
function extractEditedFilesFromPatchText(text: string, cwd: string | undefined): string[] {
  const out: string[] = [];
