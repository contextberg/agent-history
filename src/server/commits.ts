...
export async function aggregateCommits(sessions: AgentSession[]): Promise<CommitWithLinks[]> {
  ...
  for (const s of sessions) {
    ...
    const bases = expandedBases([s.cwd, ...(s.additionalCwds ?? [])]);
    for (const t of s.turns) {
      for (const f of t.touchedFiles ?? []) {
        ...
        if (path.isAbsolute(f)) {
          candidatePaths.add(path.dirname(f));
        } else {
          for (const base of bases) candidatePaths.add(path.dirname(path.resolve(base, f)));
        }
      }
    }
  }
    ...
    for (const f of t.touchedFiles ?? []) {
      if (path.isAbsolute(f)) {
        ...
        for (const base of bases) {
          if (pathToRepo.get(path.dirname(path.resolve(base, f))) === repo) return true;
        }
      }
    }
  }
