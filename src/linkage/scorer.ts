...
export function scoreSessionCommit(
  ...
  if (authoringReference) {
    ...
    // File overlap by itself is a weak signal for documentation and config
    // files that are touched repeatedly across a project. Likewise, a commit
    // shortly after a same-repo session is only useful when the source also
    // recorded edited files, a matching branch, or a concrete SHA reference.
    if (parts.time === 0 && parts.files > 0) total = Math.min(total, 0.4);
    if (parts.files === 0 && parts.branch === 0 && parts.time < 1) total = Math.min(total, 0.4);
  }
  ...
  for (const t of session.turns) {
    ...
    for (const f of t.touchedFiles) {
      if (path.isAbsolute(f)) {
        ...
        for (const base of bases) out.add(path.resolve(base, f));
      }
    }
  }
  return out;
